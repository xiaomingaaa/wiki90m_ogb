...
class MultiLinear(torch.nn.Module):
    def __init__(self, input_dim, out_dim, hidden_dim=200, hidden_layer=1) -> None:
    ...
    def _construct_net(self):
        self.net.append(nn.Linear(self.in_dim,self.hidden_dim))
        for _ in range(self.hidden_layer):
            self.net.append(nn.Linear(self.hidden_dim, self.hidden_dim))
        self.net.append(nn.Linear(self.hidden_dim,self.out_dim))
    def forward(self, x):
...
class MLP(torch.nn.Module):
    def __init__(self, input_entity_dim, entity_dim, input_relation_dim, relation_dim):
        ...
        ### use multiple linear layers
        self.transform_e_net = MultiLinear(input_entity_dim, entity_dim)
        self.transform_r_net = MultiLinear(input_relation_dim, relation_dim)
    ...
    def reset_parameters(self):
        ...
        self.transform_e_net.apply(weight_init)
        self.transform_r_net.apply(weight_init)
